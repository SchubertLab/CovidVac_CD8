...
def extract_clonotype_information(adata, information, name):
    clono_info = adata.obs[[f'IR_VJ_1_{information}', f'IR_VJ_2_{information}',
                            ...
                            'clone_id']].copy()
    clono_info = clono_info.sort_values('clone_id')
    ...
    def assign_clonotype_seqs(chains):
        if len(chains) == 1:
            ...
            return f'{alpha} {beta}'
        beta_1 = chains[f'IR_VDJ_1_{information}'].value_counts().sort_index(ascending=False).sort_values(ascending=False)
        beta_joint = pd.concat([chains[f'IR_VDJ_1_{information}'], chains[f'IR_VDJ_2_{information}']]).value_counts().sort_index(ascending=False).sort_values(ascending=False)

        if len(beta_joint) > 1 and beta_joint.iloc[0] == beta_joint.iloc[1]:
            beta = beta_1.index[0]
        else:
            beta = beta_joint.index[0]

        alpha_1 = chains[f'IR_VJ_1_{information}'].value_counts().sort_index(ascending=False).sort_values(ascending=False)
        alpha_joint = pd.concat([chains[f'IR_VJ_1_{information}'], chains[f'IR_VJ_2_{information}']]).value_counts().sort_index(ascending=False).sort_values(ascending=False)

        if len(alpha_joint) > 1 and alpha_joint.iloc[0] == alpha_joint.iloc[1]:
            alpha = alpha_1.index[0]
        else:
            alpha = alpha_joint.index[0]
        return f'{alpha} {beta}'
